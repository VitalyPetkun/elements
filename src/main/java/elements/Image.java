...
public class Image extends Element implements IImage {
    ...
    public void save(String imageName, String path) {
        try {
            ((JavascriptExecutor) AqualityServices.getBrowser().getDriver()).executeScript("arguments[0].scrollIntoView();", AqualityServices.getBrowser().getDriver().findElement(this.getLocator()));
            Thread.sleep(2000);
    ...
    public String setFullFileName(String imageName, String path) {
        if (fullFileName == null)
            fullFileName = System.getProperty("user.dir").concat("\\").concat(path).concat(imageName).concat(".").concat(FORMAT_NAME);
        return fullFileName;
    }
    private URL getUrl() {
        if (url == null) {
